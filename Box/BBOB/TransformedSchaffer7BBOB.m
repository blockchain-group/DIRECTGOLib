function y = TransformedSchaffer7BBOB(x)
% -------------------------------------------------------------------------
...
if isempty(fopt) || isempty(xopt) || dim ~= length(x) || funid ~= 18 ...
    ...
    linearTF = compute_rotation(18, dim)*diag(sqrt(1000).^linspace(0, 1 , dim));
...
function fmin = get_fmin(~)
    fmin = min([1000, max([-1000, (round(100*100*gauss(1, 18)/gauss(1, 18 + 1))/100)])]);
end
