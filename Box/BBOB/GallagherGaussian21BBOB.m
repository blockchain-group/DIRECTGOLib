function y = GallagherGaussian21BBOB(x)
% -------------------------------------------------------------------------
...
if isempty(fopt) || dim ~= length(x) || funid ~= 22 || isempty(xlocal) ...
...
function fmin = get_fmin(~)
    fmin = min([1000, max([-1000, (round(100*100*gauss(1, 22)/gauss(1, 22 + 1))/100)])]);
end

function xmin = get_xmin(nx)
    fac2 = 0.98;
    rotation = compute_rotation(22, nx);
    xlocal = fac2*reshape(10*unif(nx*21, 22) - 5, [nx, 21])'*rotation;
    xlocal(1, :) = 0.8*xlocal(1, :);
