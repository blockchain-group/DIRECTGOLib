...
if isempty(fopt) ||isempty(xopt) || dim ~= length(x) || funid ~= 2
    dim = length(x);
    xopt = get_xmin(dim);
    fopt = get_fmin(dim);
    funid = 2;
...
y = sum((1e+6).^linspace(0, 1, dim).*(z.^2)) + fopt;
end
...
function fmin = get_fmin(~)
    fmin = min([1000, max([-1000, (round(100*100*gauss(1, 2)/gauss(1, 2 + 1))/100)])]);
end
