...
if isempty(fopt) ||isempty(xopt) || dim ~= length(x) || funid ~= 1
    dim = length(x);
    xopt = get_xmin(dim);
    fopt = get_fmin(dim);
    funid = 1;
...
function fmin = get_fmin(~)
    fmin = min([1000, max([-1000, (round(100*100*gauss(1, 1)/gauss(1, 1 + 1))/100)])]);
end

function xmin = get_xmin(nx)
    xmin = 8*floor(1e+4*unif(nx, 1))/1e+4 - 4;
    xmin(xmin == 0) = -1e-5;
end