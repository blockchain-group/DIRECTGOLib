function y = BentCigarBBOB(x, inst)
% -------------------------------------------------------------------------
...
if isempty(fopt) || isempty(xopt) || dim ~= length(x) || funid ~= inst || isempty(arrexpo) || isempty(R)
    dim = length(x);
...
function xmin = get_xmin(nx, inst)
    ...
    xmin = 8*floor(1e+4*unif(nx, inst + 1e+6))/1e+4 - 4;
    xmin(xmin == 0) = -1e-5;
end