...
if isempty(fopt) || isempty(xopt) || dim ~= length(x) || funid ~= 23 || isempty(linearTF)
    dim = length(x);
    xopt = get_xmin(dim);
    fopt = get_fmin(dim);
    funid = 23;
    linearTF = (compute_rotation(23, dim)*diag(sqrt(100).^linspace(0, 1, dim)))*compute_rotation(23 + 1e+6, dim);
end
...
function fmin = get_fmin(~)
    fmin = min([1000, max([-1000, (round(100*100*gauss(1, 23)/gauss(1, 23 + 1))/100)])]);
end
