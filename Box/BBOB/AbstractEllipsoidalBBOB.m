function y = AbstractEllipsoidalBBOB(x, inst)
% -------------------------------------------------------------------------
% MATLAB coding by: Linas Stripinis
% Name:
%   AbstractEllipsoidalBBOB.m
%
% References:
...
persistent xopt dim fopt funid scales R
if isempty(fopt) || isempty(xopt) || dim ~= length(x) || funid ~= inst || isempty(scales) || isempty(R)
    dim = length(x);
    xopt = get_xmin(dim, inst);
    fopt = get_fmin(dim);
    funid = inst;
    scales = (1e+6).^linspace(0, 1, dim);
