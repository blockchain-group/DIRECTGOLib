...
void cec17_test_func(double *, double *,int,int,int, double *, double *, int *);

void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]) 
{
	int  cf_num=10,m, n,func_num, i, *shuffle, size_shuffle;
	double  *f, *x, *M, *shift, *shuffle_double;
	if ((nrhs < 2) || (nlhs < 1))
    {
	...
	n = mxGetM (prhs[0]);
	if (!(n==2||n==5||n==6||n==10||n==20||n==30||n==50||n==100))
    {
		mexPrintf ("usage: f = cec17_func(x, func_num);\n");
		mexErrMsgTxt ("Error: Test functions are only defined for D=2,5,10,20,30,50,100.");
    }
	m = mxGetN (prhs[0]);
	x = mxGetPr (prhs[0]);
	...
	M = mxGetPr(prhs[2]);
    shift = mxGetPr(prhs[3]); 
    shuffle_double = mxGetPr(prhs[4]);
    size_shuffle = sizeof(shuffle_double);
    /*mexPrintf("size %i \n",size_shuffle);*/
    if (func_num>=1&&func_num<=20)
    {
        shuffle=(int *)malloc(n*sizeof(int));
        for(i=0;i<n;i++)
			{
				/*mexPrintf("%f\n",shuffle_double[i]);*/
                shuffle[i] = int(shuffle_double[i]);
			} 
    }
    else if (func_num==29||func_num==30)
    {
        shuffle=(int *)malloc(n*cf_num*sizeof(int));
        for(i=0;i<n*cf_num;i++)
			{
				/*mexPrintf("%f\n",shuffle_double[i]);*/
                shuffle[i] = int(shuffle_double[i]);
			} 
	...
	f = mxGetPr (plhs[0]);
	cec17_test_func(&x[0], &f[0], n,m,func_num,&M[0], &shift[0], &shuffle[0]);
    if ((func_num >= 1 && func_num <= 20) | (func_num==29||func_num==30))
    {
        free(shuffle);
    }
    f[0] = f[0]+0.0;
...
void cec17_test_func(double *x, double *f, int nx, int mx,int func_num, double *M, double *OShift, int *SS)
{
	int cf_num=10,i,j;
	if (ini_flag==1)
	{
	...
	if (ini_flag==0)
	{
		...
		if (!(nx==2||nx==5||nx==6||nx==10||nx==20||nx==30||nx==50||nx==100))
		{
			printf("\nError: Test functions are only defined for D=2,5,10,20,30,50,100.\n");
		}
