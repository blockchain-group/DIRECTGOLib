...
void cec14_test_func(double *, double *,int,int,int, double *, double *, int *);

void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[]) 
{
	int  m, n,func_num, i, *shuffle, size_shuffle,cf_num=10;
	...
	n = mxGetM (prhs[0]);
	if (!(n==2||n==5||n==10||n==20||n==30||n==50||n==100))
    {
		mexPrintf ("usage: f = cec14_func(x, func_num);\n");
	...
	f = mxGetPr (plhs[0]);
	cec14_test_func(&x[0], &f[0], n,m,func_num,&M[0], &shift[0], &shuffle[0]);
    free(shuffle);
    f[0] = f[0]+0.0;
}


void cec14_test_func(double *x, double *f, int nx, int mx,int func_num, double *M, double *OShift, int *SS)
{
	int cf_num=10,i,j;
	if (ini_flag==1)
	{
	...
	if (ini_flag==0)
	{
		...
		if (!(nx==2||nx==5||nx==10||nx==20||nx==30||nx==50||nx==100))
		{
