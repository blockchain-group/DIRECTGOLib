...
% [2] Konstantin Dietrich, Diederick Vermetten, Carola Doerr, and Pascal 
%     Kerschke. 2024. Impact of Training Instance Selection on Automated 
%     Algorithm Selection Models for Numerical Black-box Optimization. In 
%     Proceedings of the Genetic and Evolutionary Computation Conference 
%     (GECCO '24). Association for Computing Machinery, New York, NY, USA, 
%     1007–1016. https://doi.org/10.1145/3638529.3654100
%--------------------------------------------------------------------------
...
% Calculate the Manhattan distances between ELA features
ManhDist = manhattan_distances( data_norm );

% Greedy approach
represent = zeros( 1, Instances_to_select ); 
all_ids = 1:size(data_norm, 1);
m_s = min( ManhDist );
[maxval, maxpos] = max( m_s );
represent(1) = maxpos;
rest_ids = setdiff( all_ids, represent(1) );
for j = 2:Instances_to_select
    m_s = min( ManhDist(represent(1:(j - 1)), rest_ids), [], 1 );
    [maxval, maxpos] = max( m_s );
    represent(j) = rest_ids(maxpos);
    rest_ids = setdiff( rest_ids, represent(j) );
end

DIRECTGOLib_Results = DIRECTGOLib_ELA([1, represent + 1], :);
...
save( 'DIRECTGOLib_settings_md.mat', 'DIRECTGOLib_Results' );
...
function D = manhattan_distances(X)
    ...
    for i = 1:n
        D(i, i) = nan;
        for j = i + 1:n
            [D(i, j), D(j, i)] = deal( sum( abs( X(i, :) - X(j, :) ) ) );
        end
    end
end